...
var Topological = function(){	
	...
	this.Topological = function(G){
		...
		if((this.cyclefinder.hasCycle())!=true){//若是,则找出图的顶点以逆后序排列,输出至order
			...
			this.order=this.dfs.get_reversePost();
		}
	}
...
//---------------------算法(第四版)  386页练习4.2.7,有向图的入度,出度---------------------//
var Degrees = function(){
	this.Degrees = function(G){
		this.G=G;
	}
	
	this.inDegree = function(v){//求v的入度
		var count=0;
		for(var i=0;i<this.G.adj.length;i++){
			var tempNode=this.G.adj[i].first;
			while(tempNode!=null){
				if(tempNode.item==v){
					count++;
				}
	...
	this.outDegree = function(v){//求v的出度
		var count=0;
		var tempNode=this.G.adj[v].first;
		while(tempNode!=null){
			tempNode=tempNode.next;
			count++;
		}
	...
	this.sources = function(){//求所有起点(入度为0)的集合
		var arr=new Array();
		for(var i=0;i<this.G.adj.length;i++){
	...
	this.sinks = function(){//求所有终点(出度为0)的集合
		var arr=new Array();
		for(var i=0;i<this.G.adj.length;i++){
	...
	this.isMap = function(){//判断图是否映射(每个顶点的出度皆为1)
		for(var i=0;i<this.G.adj.length;i++){
